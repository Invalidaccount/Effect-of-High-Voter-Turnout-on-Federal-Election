...
raw_data <- read_csv("AAycpXmh.csv")
...
gss <- gss %>%
  ...
  mutate(education = case_when(
    ...
    ~ "Dipolma below bachelor",
    education == "University certificate, diploma or degree above the bach..." 
    ~ "Above bachelor"
  ))
gss <- gss %>% drop_na(education)


gss <- gss %>%
  rowwise() %>%
  mutate(place_birth_canada = case_when(
    place_birth_canada == "Don't know" ~ "DN",
    place_birth_canada == "Born in Canada" ~ "In Canada",
    place_birth_canada == "Born outside Canada" ~ "Outside Canada"
  ))
gss <- gss %>% drop_na(place_birth_canada)
...
gss <- gss %>%
  ...
  mutate(language_home = case_when(
    ...
    language_home == "English and French" ~ "EN",
    language_home == "Non-official languages" ~ "Non-Official",
    language_home == "English and non-official language" ~ "EN",
    language_home == "French and non-official language" ~ "FR",
    language_home == "Multiple non-official languages" ~ "Non-Official",
    language_home == "English, French and non-official language" ~ "FR"
  ))
gss <- gss %>% drop_na(language_home)

gss <- gss %>% drop_na(feelings_life)
